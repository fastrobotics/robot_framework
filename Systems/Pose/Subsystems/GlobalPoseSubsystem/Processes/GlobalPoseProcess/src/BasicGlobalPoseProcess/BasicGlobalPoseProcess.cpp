...
namespace fast::rf::PoseSystem::GlobalPoseSubsystem {
    bool BasicGlobalPoseProcess::update(double current_time_sec) {
        bool status = base_update(current_time_sec);
        if (status == false) {
            return false;
        }
        return true;
    }
    bool BasicGlobalPoseProcess::new_GlobalPositionSensorMsg(uint8_t index,
                                                             SensorMsgs::GlobalPositionSensorMsg gps_data) {
