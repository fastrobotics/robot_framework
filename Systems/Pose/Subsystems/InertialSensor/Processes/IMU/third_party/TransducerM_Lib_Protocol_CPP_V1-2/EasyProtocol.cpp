...
#else
#define EP_QUEUE_SIZE_ (iDS * 4)  // Smaller queue size for embedded syst.
#endif
    HEAD_1_ = (char)0xaa;
 ...
 */
int EasyProtocol::Init(int iDataSize,  ///< [INPUT]
                       int oDataSize   ///< [INPUT]
) {
    // totalMem = 0;                                                  //new
...
void EasyProtocol::SetChecksumOption(char option) {
    ...
    CHECKSUM_LENGTH_ = 1;
    if ((option == EP_CHECKSUM_2_BYTES_SUM_) || (option == EP_CHECKSUM_2_BYTES_CRC_)) {
 ...
 */
#ifdef EP_TURN_ON_STATISTICS_
int EasyProtocol::TotalBytesOfMemoryUsed() { return totalMem; }

int EasyProtocol::GetStatistic_Recv_Byte_Total() { return sRecvByte_Total; }

float EasyProtocol::GetStatistic_Recv_Byte_BadHeadRate() { return ((float)sRecvByte_BadHead / sRecvByte_Total); }

int EasyProtocol::GetStatistic_Recv_Pkg_Total() { return sRecvPkg_Total; }

float EasyProtocol::GetStatistic_Recv_Pkg_OmitRate() { return ((float)sRecvPkg_Omit / sRecvPkg_Total); }

float EasyProtocol::GetStatistic_Recv_Pkg_BadRate() { return ((float)sRecvPkg_Bad / sRecvPkg_Total); }

float EasyProtocol::GetStatistic_Recv_Pkg_GoodRate() { return ((float)sRecvPkg_Good / sRecvPkg_Total); }

int EasyProtocol::GetStatistic_Send_Pkg_Total() { return sSendPkg; }
#endif

int EasyProtocol::GetInDataMaxSize(void) { return iDS; }

int EasyProtocol::GetOutDataMaxSize(void) { return oDS; }

int EasyProtocol::GetRoundUp(void) { return roundUpTmp; }
 ...
 */
int EasyProtocol::CreateOutputPackage(char* payloadData,   ///< [INPUT]  Payload data pointer
    ...
    // Check validity:
    if (payloadSize > MAX_PAYLOAD_SIZE_)
        return EP_FAIL_;
    if ((HEAD_LENGTH_ != 1) && (HEAD_LENGTH_ != 2))
        return EP_FAIL_;
    if ((SIZE_LENGTH_ != 1) && (SIZE_LENGTH_ != 2))
        return EP_FAIL_;
    if ((CHECKSUM_LENGTH_ != 1) && (CHECKSUM_LENGTH_ != 2))
        return EP_FAIL_;
    if (0 == outBuf)
        return EP_FAIL_;  // new
 ...
 */
uint16 EasyProtocol::Checksum_Generate(char* data,     ///< [INPUT] Data relevant for generating checksum.
                                       int dataLength  ///< [INPUT]
) {
    uint16 checkSum = 0;

    if ((CHECKSUM_OPTION_ == EP_CHECKSUM_1_BYTE_SUM_) || (CHECKSUM_OPTION_ == EP_CHECKSUM_2_BYTES_SUM_)) {
        int i;
...
#endif
    if (checksumToBeVerified == checkSum)
        return EP_SUCC_;
    else
        return EP_FAIL_;
}
 ...
 *         EP_SUCC_
 */
int EasyProtocol::UnWrapInData(char* packageData,   ///< [INPUT]
    ...
    if (CHECKSUM_LENGTH_ == 1) {
        ...
        for (int i = 0; i < 2; i++) {
            *(((uint8_t*)(&recvChecksum)) + i) =
                *(((uint8_t*)(packageData + payloadSize + EP_PKG_MODIFIER_SIZE_ - CHECKSUM_LENGTH_)) + i);
        }
    }
        ...
        if (p == inBuf) {
            ...
            if (c == (char)HEAD_2_)
                *p++ = c;
                ...
                p = inBuf;
        } else if ((p >= (inBuf + HEAD_LENGTH_)) && (p < (inBuf + HEAD_LENGTH_ + SIZE_LENGTH_))) {
            *p++ = c;
            if (p >= (inBuf + HEAD_LENGTH_ + SIZE_LENGTH_)) {
                if (SIZE_LENGTH_ == 1) {
                    ...
                    if (!HEAD_LENGTH_MSB_AHEAD_) {
                        ...
                        declaredPayloadSize = ((uint16)(*((uint8*)(p - SIZE_LENGTH_))) << 8) +
