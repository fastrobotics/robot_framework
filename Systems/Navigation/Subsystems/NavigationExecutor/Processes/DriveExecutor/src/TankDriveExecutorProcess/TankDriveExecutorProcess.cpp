...
TankDriveExecutorProcess::new_cmd(GeometryMsgs::TwistMsg cmd) {
...
TankDriveData TankDriveExecutorProcess::convert(GeometryMsgs::TwistMsg twist) {
  ...
  double m_left =
      left_channel_config.max_value - left_channel_config.min_value / 2.0;
  double m_right =
      right_channel_config.max_value - right_channel_config.min_value / 2.0;
  ...
  if (left_channel_config.max_value > left_channel_config.min_value) {
    left_clipped = clip(left_clipped, left_channel_config.min_value,
                        left_channel_config.max_value);
  } else {
    left_clipped = clip(left_clipped, left_channel_config.max_value,
                        left_channel_config.min_value);
  }

  double right_clipped = right_scaled;
  if (right_channel_config.max_value > right_channel_config.min_value) {
    right_clipped = clip(right_clipped, right_channel_config.min_value,
                         right_channel_config.max_value);
  } else {
    right_clipped = clip(right_clipped, right_channel_config.max_value,
                         right_channel_config.min_value);
  }
...
double TankDriveExecutorProcess::clip(double value, double min_value,
                                      double max_value) {
  double output = value;
  if (value > max_value) {
    output = max_value;
