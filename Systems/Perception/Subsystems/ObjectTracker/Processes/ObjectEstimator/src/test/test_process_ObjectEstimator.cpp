...
#include <IObjectEstimatorProcess.hpp>
#include <Infrastructure/Logger.hpp>

using namespace fast::rf::PerceptionSystem::ObjectTrackerSubsystem::ObjectEstimator;
class TestObjectEstimatorProcessInterface : public IObjectEstimatorProcess {
   public:
...
TEST(TestObjectEstimatorProcessInterface, InterfaceTests) {
    TestObjectEstimatorProcessInterface SUT;
    ASSERT_TRUE(SUT.init());
    ASSERT_EQ(SUT.getDiagnostics().size(), 0);
    ASSERT_FALSE(SUT.update(0.0));
}
class TestBaseObjectEstimatorProcess : public BaseObjectEstimatorProcess {
   public:
    TestBaseObjectEstimatorProcess() : BaseObjectEstimatorProcess() {}
    bool init() override {
    ...
    bool update(double current_time_sec) override { return BaseObjectEstimatorProcess::update(current_time_sec); }
    std::string pretty() {
...
TEST(BaseObjectEstimatorProcess, BasicAssertions) {
    TestBaseObjectEstimatorProcess SUT;
    ASSERT_TRUE(SUT.init());
