...
class TestDepthCameraFeatureHandlerProcessInterface : public IDepthCameraFeatureHandlerProcess {
   public:
...
TEST(TestDepthCameraFeatureHandlerProcessInterface, InterfaceTests) {
    TestDepthCameraFeatureHandlerProcessInterface SUT;
    ASSERT_TRUE(SUT.init());
    ASSERT_EQ(SUT.getDiagnostics().size(), 0);
    ASSERT_FALSE(SUT.update(0.0));
}
class TestBaseDepthCameraFeatureHandlerProcess : public BaseDepthCameraFeatureHandlerProcess {
   public:
    TestBaseDepthCameraFeatureHandlerProcess() : BaseDepthCameraFeatureHandlerProcess() {}
    bool init() override {
    ...
    bool update(double current_time_sec) override {
        return BaseDepthCameraFeatureHandlerProcess::update(current_time_sec);
    }
    std::string pretty() {
...
TEST(BaseDepthCameraFeatureHandlerProcess, BasicAssertions) {
    TestBaseDepthCameraFeatureHandlerProcess SUT;
    ASSERT_TRUE(SUT.init());
