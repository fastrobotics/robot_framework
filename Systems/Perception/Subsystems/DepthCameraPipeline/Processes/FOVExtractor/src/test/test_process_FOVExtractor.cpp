...
#include <IFOVExtractorProcess.hpp>
#include <Infrastructure/Logger.hpp>

using namespace fast::rf::PerceptionSystem::DepthCameraPipelineSubsystem::FOVExtractor;
class TestFOVExtractorProcessInterface : public IFOVExtractorProcess {
   public:
...
TEST(TestFOVExtractorProcessInterface, InterfaceTests) {
    TestFOVExtractorProcessInterface SUT;
    ASSERT_TRUE(SUT.init());
    ASSERT_EQ(SUT.getDiagnostics().size(), 0);
    ASSERT_FALSE(SUT.update(0.0));
}
class TestBaseFOVExtractorProcess : public BaseFOVExtractorProcess {
   public:
    TestBaseFOVExtractorProcess() : BaseFOVExtractorProcess() {}
    bool init() override {
    ...
    bool update(double current_time_sec) override { return BaseFOVExtractorProcess::update(current_time_sec); }
    std::string pretty() {
...
TEST(BaseFOVExtractorProcess, BasicAssertions) {
    TestBaseFOVExtractorProcess SUT;
    ASSERT_TRUE(SUT.init());
