...
class TestSensorFuserProcessInterface : public ISensorFuserProcess {
   public:
...
TEST(TestSensorFuserProcessInterface, InterfaceTests) {
    TestSensorFuserProcessInterface SUT;
    ASSERT_TRUE(SUT.init());
    ASSERT_EQ(SUT.getDiagnostics().size(), 0);
    ASSERT_FALSE(SUT.update(0.0));
}
class TestBaseSensorFuserProcess : public BaseSensorFuserProcess {
   public:
    TestBaseSensorFuserProcess() : BaseSensorFuserProcess() {}
    bool init() override {
...
TEST(BaseSensorFuserProcess, BasicAssertions) {
