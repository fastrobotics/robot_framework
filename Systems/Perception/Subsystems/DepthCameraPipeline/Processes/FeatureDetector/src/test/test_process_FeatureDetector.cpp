...
#include <IFeatureDetectorProcess.hpp>
#include <Infrastructure/Logger.hpp>

using namespace fast::rf::PerceptionSystem::DepthCameraPipelineSubsystem::FeatureDetector;
class TestFeatureDetectorProcessInterface : public IFeatureDetectorProcess {
   public:
...
TEST(TestFeatureDetectorProcessInterface, InterfaceTests) {
    TestFeatureDetectorProcessInterface SUT;
    ASSERT_TRUE(SUT.init());
    ASSERT_EQ(SUT.getDiagnostics().size(), 0);
    ASSERT_FALSE(SUT.update(0.0));
}
class TestBaseFeatureDetectorProcess : public BaseFeatureDetectorProcess {
   public:
    TestBaseFeatureDetectorProcess() : BaseFeatureDetectorProcess() {}
    bool init() override {
    ...
    bool update(double current_time_sec) override { return BaseFeatureDetectorProcess::update(current_time_sec); }
    std::string pretty() {
...
TEST(BaseFeatureDetectorProcess, BasicAssertions) {
    TestBaseFeatureDetectorProcess SUT;
    ASSERT_TRUE(SUT.init());
