...
class TestExampleProcessInterface : public IExampleProcess {
   public:
...
TEST(TestExampleProcessInterface, InterfaceTests) {
    TestExampleProcessInterface SUT;
    ASSERT_TRUE(SUT.init());
    ASSERT_EQ(SUT.getDiagnostics().size(), 0);
    ASSERT_FALSE(SUT.update(0.0));
}
class TestBaseExampleProcess : public BaseExampleProcess {
   public:
    TestBaseExampleProcess() : BaseExampleProcess() {}
    bool init() override {
    ...
    bool update(double current_time_sec) override { return BaseExampleProcess::update(current_time_sec); }
    std::vector<fast::rf::messages::InfrastructureMsgs::DiagnosticMsg> getDiagnostics() {
...
TEST(BaseExampleProcess, BasicAssertions) {
    TestBaseExampleProcess SUT;
    ASSERT_TRUE(SUT.init());
