...
namespace fast::rf {
    ...
    SingletonLogger::SingletonLogger(Level level, std::string name) {
    ...
    SingletonLogger::LoggerStatus SingletonLogger::LOG_DEBUG(std::string filename, uint64_t linenumber,
                                                             std::string tempstr) {
        return print_log(filename, linenumber, Level::DEBUG, tempstr);
    }
    SingletonLogger::LoggerStatus SingletonLogger::LOG_INFO(std::string filename, uint64_t linenumber,
                                                            std::string tempstr) {
        return print_log(filename, linenumber, Level::INFO, tempstr);
    }
    SingletonLogger::LoggerStatus SingletonLogger::LOG_NOTICE(std::string filename, uint64_t linenumber,
                                                              std::string tempstr) {
        return print_log(filename, linenumber, Level::NOTICE, tempstr);
    }
    SingletonLogger::LoggerStatus SingletonLogger::LOG_WARN(std::string filename, uint64_t linenumber,
                                                            std::string tempstr) {
        return print_log(filename, linenumber, Level::WARN, tempstr);
    }
    SingletonLogger::LoggerStatus SingletonLogger::LOG_ERROR(std::string filename, uint64_t linenumber,
                                                             std::string tempstr) {
        return print_log(filename, linenumber, Level::ERROR, tempstr);
    }
    SingletonLogger::LoggerStatus SingletonLogger::LOG_FATAL(std::string filename, uint64_t linenumber,
                                                             std::string tempstr) {
        return print_log(filename, linenumber, Level::FATAL, tempstr);
    }
    SingletonLogger::LoggerStatus SingletonLogger::print_log(std::string filename, uint64_t linenumber, Level level,
                                                             std::string tempstr) {
        time_t rawtime;
        struct tm* timeinfo;
        char datebuffer[80];
            ...
            if (level >= verbosity) {
                ...
                switch (level) {
                    ...
                    case Level::INFO:
                        ...
                        break;
                    case Level::NOTICE:
                        log_file << "[" << datebuffer << "]: NOTICE: " << swcode_info << " " << tempstr << std::endl;
                        if (console_print) {
