...
#include <algorithm>
namespace fast::rf {
    Logger::Logger(Level level, std::string name) : logger_ok(false) {
        verbosity = level;
        line_counter = 0;
        if (name.at(0) == '/') {
    ...
    Logger::LoggerStatus Logger::print_log(std::string filename, uint64_t linenumber, Level level,
        ...
        if (logger_ok == false) {
            printf("%s[%s %s]: WARN: LOGGER HAS FAILED: %s%s\n", YELLOW_FOREGROUND.c_str(), datebuffer,
                   node_name.c_str(), tempstr.c_str(), END_COLOR.c_str());
            ...
            if (level >= verbosity) {
                ...
                switch (level) {
                    ...
                    case Level::NOTICE:
                        ...
                        if (console_print) {
                            printf("%s[%s %s]: NOTICE: %s%s\n", GREEN_FOREGROUND.c_str(), datebuffer, node_name.c_str(),
                    ...
                    case Level::WARN:
                        ...
                        if (console_print) {
                            printf("%s[%s %s]: WARN: %s%s\n", YELLOW_FOREGROUND.c_str(), datebuffer, node_name.c_str(),
                                   tempstr.c_str(), END_COLOR.c_str());
                        }
#ifdef ROS_INSTALLED
                        if (use_ROS_logger == true) {
                    ...
                    case Level::ERROR:
                        ...
                        if (console_print) {
                            printf("%s[%s %s]: ERROR: %s%s\n", RED_FOREGROUND.c_str(), datebuffer, node_name.c_str(),
                                   tempstr.c_str(), END_COLOR.c_str());
                        }
#ifdef ROS_INSTALLED
                        if (use_ROS_logger == true) {
